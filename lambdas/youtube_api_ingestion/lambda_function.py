...
# ── Config ───────────────────────────────────────────────────────────────────
# Retrieve YouTube API key from AWS Secrets Manager
YOUTUBE_API_KEY_SECRET_ARN = os.environ["YOUTUBE_API_KEY_SECRET"]
try:
    secret_response = secretsmanager_client.get_secret_value(SecretId=YOUTUBE_API_KEY_SECRET_ARN)
    secret_json = json.loads(secret_response.get("SecretString", ""))
    API_KEY = secret_json.get("youtube_api_key", "")
    if not API_KEY:
        raise ValueError("YouTube API key secret is empty")
except Exception as e:
    logger.error(f"Failed to retrieve YouTube API key from Secrets Manager: {e}")
    raise
...
def log_http_error(prefix: str, error: HTTPError) -> None:
    """Log the HTTP status and response body returned by YouTube."""
    try:
        error_body = error.read().decode("utf-8")
    except Exception:
        error_body = "<unable to read error body>"

    logger.error(f"{prefix}: {error} | response body: {error_body}")
...
def lambda_handler(event, context):
    ...
    for region in REGIONS:
        ...
        try:
            ...
            log_http_error(f"  API error for {region} trending", e)
            results["failed"].append({"region": region, "type": "trending", "error": str(e)})
            continue
        except URLError as e:
            logger.error(f"  Network error for {region} trending: {e}")
            results["failed"].append({"region": region, "type": "trending", "error": str(e)})
            continue
        except Exception as e:
        ...
        try:
            ...
            log_http_error(f"  API error for {region} categories", e)
            results["failed"].append({"region": region, "type": "categories", "error": str(e)})
            continue
        except URLError as e:
            logger.error(f"  Network error for {region} categories: {e}")
            results["failed"].append({"region": region, "type": "categories", "error": str(e)})
            continue
